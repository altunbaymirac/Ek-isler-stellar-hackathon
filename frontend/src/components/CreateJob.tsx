  ...
  const submit = async () => {
    ...
    try {
      ...
      const { result, hash } = await createJob(signer, {
        client: clientAddr,
        totalUsdc: amount,
        stakeholders: stakeholders.map((s) => ({ address: s.address, share_bps: Math.round(s.percent * 100) })),
      ...
      <section className="card stack">
        <div>
          ...
          <p className="muted small" style={{ margin: 0 }}>
            ...
            <b>her çalışan kendi cüzdanıyla onaylamadan</b> müşteri para yatıramaz.
          </p>
        </div>
        ...
        <label className="field">
          ...
          <span className="small muted" style={{ fontWeight: 400 }}>
            Müşteri bu tarihe kadar işi onaylamaz ya da ihaleciyle birlikte iptal etmezse ödeme otomatik dağıtılabilir.
          </span>
        </label>
      </section>
